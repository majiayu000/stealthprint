# omen-alpha-fingerprint

对 opencode zen 网关背后的匿名模型 `omen-alpha` 做的完整指纹分析，附可复用的测试工具包（`omenprint.py`），可对任意 stealth 模型复跑同一套流程。

> **English TL;DR:** `omen-alpha` served via `https://opencode.ai/zen/go/v1` uses the **GLM-5 tokenizer** (24/24 probes exact match, MAE 0.00, vs o200k 11/24). Real verified context ≥ **969K tokens** (not the rumored 500K), bounded only by a 4.5 MiB gateway body limit. The serving stack is a **Rust** ("Console Go") OpenAI-compatible gateway, *not* the official Zhipu API (no `[1210]/[1214]` codes, accepts `developer` role). Vision is **not reliably functional**: multimodal requests fail ~50% of the time (heterogeneous backend pool), image cost is a constant +18 tokens regardless of size, and color/dimension answers are hallucinated.
...
完整原始数据见 [`data/measurements.json`](data/measurements.json)。
...
5. **视觉真值**：纯色图颜色问答 + 图像 token 开销随尺寸的斜率。恒定开销 + 颜色答错 = 假管线。
