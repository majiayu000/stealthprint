...
- **编程任务简/繁直测（09-17 五轮，用户点题「简单编程 vs 复杂编程」）**：5 领域配对（字符串/搜索/DP/图/解析）——元音计数 vs 正则匹配 DP、二分 vs 两有序数组中位数 O(log)、爬楼梯 vs 编辑距离、判环 vs Dijkstra、CSV 单行 vs 四则运算+括号求值器——**全部独立 oracle 机器判分**（re.fullmatch / statistics.median / 记忆化递归 / Bellman-Ford / csv 模块 / eval，各 1500~2000 随机对拍）：**10/10 实质输入全过**，唯一分歧是空 CSV 行返回 `['']` vs csv 模块 `[]`（两边都是公认惯例，非错误）。简繁零质量差。判分中两处初始 FAIL 均为**本人 oracle 的 bug**（dijkstra oracle 把有向边排序成无向、csv 生成器产出未转义畸形行）——模型代码在 oracle 修复后原样通过。计数通道：10/10 全落 B 族且 9/10 恰为本地 llama3−2（本 epoch B 偏移稳定），5/5 对内简繁同通道。隐形思考：简单代码中位数 3 个隐形 token（基本不思考），复杂代码中位数 211，其中**两数组中位数题 599 个隐形 token（ct 841 对可见 242）为全案最高单次隐形计费**——对比四轮补 LPS 的 0 隐形，思考深度逐任务自适应，非难度门槛。
- **自由生成实验（09-17 六轮傍晚，用户点题「让模型自由发挥做个雷电游戏，Claude Code 编排」）**：两次干净的 HTTP 200 包 **504 "Provider timed out after 300158ms"** 实锤免费档 24K token 一次性生成跑不进 OpenRouter 固定 **300s provider 墙**——改**分段生成（每段 ~3-4K completion token：设计 511 → A 2989 → B 3776 → C 3637）**后顺利完成，B 段带着 3.5K token 的 A 段上下文正确复用。自由发挥被兑现——自研游戏**《霆翼：深空突袭》**：擦弹蓄雷、Web Audio 合成音效、连杀倍率、无伤击破 Boss 召唤小猫编队彩蛋、localStorage 最高分、触屏支持、prefers-reduced-motion 无障碍适配。行为数据点：不带代码上下文要它写收尾段时，模型**拒绝盲写**（「我的上下文里只有设计清单——直接写续段无法保证变量/接口/HTML 一致性，请先把代码贴给我」），当场抓到编排方裁剪上下文；喂全代码（pt 6353）后正确收尾到 `</html>`。验证：394 行 / 26KB 单文件 HTML，`node --check` 通过 + 桩环境运行时冒烟通过——中途的「$ 重复声明」是本人冒烟脚本把 JS 拼了两遍，**模型代码零修复轮直接交付**。计数：设计 prompt pt=107 = 本地 91 + 16——**傍晚 A（llama3 尺度）通道回归**，继下午整段 B 主导之后时间漂移故事再添一笔。
