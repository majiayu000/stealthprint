...
12. **Endpoint identity drifts across epochs; single-epoch sampling misjudges**: union's counting-channel dominance flipped whole-epoch within hours (llama3 counter → B channel) while generation-side fingerprints (ct modes, echo fidelity) stayed constant — vocab "species" verdicts must be stamped with their sampling epoch. The "routes by length/task" concern was tested via 8K/32K/128K embedding and task framings: no content-axis routing; **the instability axis is time**.
